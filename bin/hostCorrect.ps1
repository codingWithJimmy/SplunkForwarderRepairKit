...
### Compare values to actual host value and flag accordingly if they exist
$INPUTS_EXISTS=$(Test-Path "$SPLUNK_LOCAL\inputs.conf" -PathType Leaf)
if ($INPUTS_EXISTS -eq "True") {
...
if ($INPUTS_EXISTS -eq "True") {
    ...
    Set-Content "$SPLUNK_LOCAL\inputs.conf" "[default]
