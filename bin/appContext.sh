...
APP_NAME=$(echo ${SCRIPT_PATH} | sed "s|${SPLUNK_HOME}/etc/apps/||" | sed "s|/bin/${SCRIPT_NAME}||")
...
if [ "${SCRIPT_NAME}" = "btoolOutput.sh" ]; then
  BTOOL_INPUT=$(${SPLUNK_HOME}/bin/splunk btool --app=${APP_NAME} inputs list "script://./bin/btoolOutput.sh $1" | grep include_default)
  INCLUDE_DEFAULT=$(echo ${BTOOL_INPUT} | awk '{print $3}')
fi
